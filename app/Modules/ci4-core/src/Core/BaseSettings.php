...
class BaseSettings
{
   ...
   public function get(string $key, ?string $context = null)
   {
       ...
       foreach ($this->handlers as $name => $handler) {
            ...
            if ($handler->has($class, $property, $context.$contextUser)) {
                return $handler->get($class, $property, $context.$contextUser);
            }
    ...
    * @return void
    */
    public function setUser(string $key, $value = null, ?string $context = null)
    {
        if (function_exists('user_id') && $userId = user_id()) {
            [$class, $property] = $this->prepareClassAndProperty($key);
    
            foreach ($this->getWriteHandlers() as $handler) {
                $handler->set($class, $property, $value, $context . '|user:' . $userId);
            }
        }else{
            throw new RuntimeException('Unable to find a Settings User handler that can store values.');
        }
    }
