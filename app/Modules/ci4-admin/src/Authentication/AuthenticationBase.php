...
/**
    ...
    public function login(User $user=null, bool $remember = false): bool
    {
        ...
        $login_id = $this->recordLoginAttempt($user->email, $ipAddress, $user->id ?? null, true);
        $this->recordSessionUser($ipAddress, $login_id, $user->id ?? null);
        ...
        $fieldSession = array('backoffice' => array(
            ...
            'groups' => $this->user->auth_groups_users,
        ));
    ...
    public function isLoggedIn(): bool
    {
        ...
        $sessionBO = session('backoffice');
    //    print_r($sessionBO); exit;
        if ($sessionBO && $userID = $sessionBO['logged_in'])
        {
    ...
    {
        $login_id =  $this->loginModel->insert([
     ...
     * @return bool|int|string
     */
    public function recordSessionUser(string $ipAddress=null, int $login_id, int $userID=null)
    {
        return model(\Adnduweb\Ci4Admin\Models\SessionUserModel::class)->insert([
            'ip_address' => $ipAddress,
            'agent' => service('request')->getUserAgent()->getAgentString(),
            'login_id' => $login_id,
            'user_id' => $userID,
